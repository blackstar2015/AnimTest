...
public class EnemyPatrolState: EnemyBaseState
{
    ...
    public override void Tick(float deltaTime)
    {
        ...
        if (stateMachine.CurrentTarget != null &&
            Vector3.Distance(stateMachine.transform.position, stateMachine.CurrentTarget.transform.position) <= stateMachine.MaxChaseDistance)
        {
            stateMachine.SwitchState(new EnemyChaseState(this.stateMachine));
        }
        StartPatrol();
    }
